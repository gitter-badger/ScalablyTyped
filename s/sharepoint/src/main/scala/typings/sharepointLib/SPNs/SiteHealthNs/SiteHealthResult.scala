...
class SiteHealthResult ()
  ...
  def get_timeStamp(): microsoftDashAjaxLib.Date with stdLib.Date = js.native
  def set_status(value: SiteHealthStatusType): scala.Unit = js.native
  def set_timeStamp(value: microsoftDashAjaxLib.Date with stdLib.Date): scala.Unit = js.native
}
