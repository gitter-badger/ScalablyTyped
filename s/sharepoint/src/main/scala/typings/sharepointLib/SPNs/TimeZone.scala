...
class TimeZone () extends ClientObject {
  ...
  def localTimeToUTC(date: microsoftDashAjaxLib.Date with stdLib.Date): DateTimeResult = js.native
  def utcToLocalTime(date: microsoftDashAjaxLib.Date with stdLib.Date): DateTimeResult = js.native
}
