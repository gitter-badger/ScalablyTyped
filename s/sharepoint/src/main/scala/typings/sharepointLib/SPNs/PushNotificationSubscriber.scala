...
class PushNotificationSubscriber () extends ClientObject {
  ...
  def get_lastModifiedTimeStamp(): microsoftDashAjaxLib.Date with stdLib.Date = js.native
  def get_registrationTimeStamp(): microsoftDashAjaxLib.Date with stdLib.Date = js.native
