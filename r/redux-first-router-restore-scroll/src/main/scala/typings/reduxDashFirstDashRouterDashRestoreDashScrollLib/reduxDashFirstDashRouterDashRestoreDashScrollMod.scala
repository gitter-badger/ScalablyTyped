package typings
package reduxDashFirstDashRouterDashRestoreDashScrollLib

import scala.scalajs.js
import scala.scalajs.js.`|`
import scala.scalajs.js.annotation._

@JSImport("redux-first-router-restore-scroll", JSImport.Namespace)
@js.native
object reduxDashFirstDashRouterDashRestoreDashScrollMod extends js.Object {
  def default(): js.Function1[
    /* history */ historyLib.historyMod.History[historyLib.historyMod.LocationState], 
    js.Object
  ] = js.native
  def default(options: reduxDashFirstDashRouterDashRestoreDashScrollLib.Anon_Manual): js.Function1[
    /* history */ historyLib.historyMod.History[historyLib.historyMod.LocationState], 
    js.Object
  ] = js.native
}

