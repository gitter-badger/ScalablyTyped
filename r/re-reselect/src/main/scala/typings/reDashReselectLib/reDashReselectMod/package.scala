...
package object reDashReselectMod {
  type OutputCachedSelector[S, R, C] = js.Function2[
    ...
    (OutputSelector[S, R, C]) with (reDashReselectLib.Anon_GetMatchingSelector[S, R, C])
  ]
  type OutputParametricCachedSelector[S, P, R, C] = js.Function2[
    /* resolver */ ParametricResolver[S, P], 
    /* optionsOrSelectorCreator */ js.UndefOr[reDashReselectLib.Options], 
    (OutputParametricSelector[S, P, R, C]) with (reDashReselectLib.Anon_GetMatchingSelectorState[S, P, R, C])
  ]
  type OutputParametricSelector[S, P, R, C] = (ParametricSelector[S, P, R]) with reDashReselectLib.Anon_ResultFunc[C]
  type OutputSelector[S, R, C] = (Selector[S, R]) with reDashReselectLib.Anon_ResultFunc[C]
