...
trait MakeAuthenticatedRequestOptions extends js.Object {
  ...
  def onAuthenticated(err: nodeLib.Error with stdLib.Error): scala.Unit = js.native
  def onAuthenticated(err: nodeLib.Error with stdLib.Error, reqOpts: DecorateRequestOptions): scala.Unit = js.native
