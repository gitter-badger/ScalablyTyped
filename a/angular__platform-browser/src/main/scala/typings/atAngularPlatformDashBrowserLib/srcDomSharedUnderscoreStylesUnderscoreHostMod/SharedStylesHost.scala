package typings
package atAngularPlatformDashBrowserLib.srcDomSharedUnderscoreStylesUnderscoreHostMod

import scala.scalajs.js
import scala.scalajs.js.`|`
import scala.scalajs.js.annotation._

@JSImport("@angular/platform-browser/src/dom/shared_styles_host", "SharedStylesHost")
@js.native
class SharedStylesHost () extends js.Object {
  def addStyles(styles: js.Array[java.lang.String]): scala.Unit = js.native
  def getAllStyles(): js.Array[java.lang.String] = js.native
  def onStylesAdded(additions: nodeLib.Set[java.lang.String] with stdLib.Set[java.lang.String]): scala.Unit = js.native
}

