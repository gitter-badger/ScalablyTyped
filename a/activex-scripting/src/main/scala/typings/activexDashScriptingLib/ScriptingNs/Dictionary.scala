...
trait Dictionary[TKey, TItem] extends js.Object {
  ...
  def Items(): activexDashInteropLib.SafeArray[TItem] with stdLib.SafeArray[TItem] = js.native
  /** Change a key to a different key. */
  def Key(Key: TKey): TKey = js.native
  /** Get an array containing all keys in the dictionary. */
  def Keys(): activexDashInteropLib.SafeArray[TKey] with stdLib.SafeArray[TKey] = js.native
